...
def scorer_auc(estimator, X, y):
    ...
    print "real", y, "predict", estimator.predict(X), "probability", predicted
...
if __name__ == '__main__':
    ...
    print "========================="
    stocks = ["000001"]
    # stocks = ["000001", "000002", "000568", "000625", "000651", "000876"]
    for stock in stocks:
        X, y = get_test_data("." + stock)
        for name, model in zip(names, models):
            print stock, name, scorer_auc(model, X, y), 1

        # newX, newY = get_newX_y(models, X, y)
        # print stock, "ridge-lasso-rf-gbdt", scorer_auc(all_model_list, newX, newY), all_model_list.coef_
        #
        # newX_rd, newY_rd = get_newX_y(models_rf_ridge, X, y)
        # print stock, "ridge-rf", scorer_auc(ridge_rf_model_list, newX_rd, newY_rd), ridge_rf_model_list.coef_
